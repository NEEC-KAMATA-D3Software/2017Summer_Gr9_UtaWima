...
cbuffer SkinningBuffer : register(b3)
{
	...
	//int DrawFrame[MAX_DRAW_NUM];
	float4 DrawFrame[MAX_DRAW_NUM];
};
...
VS_OUT VS(VS_INPUT input) {
	...
	matrix boneMat;
	boneMat  = LoadBoneTex(input.boneIndex.x, DrawFrame[input.instID].x,1) * input.weight.x;
	boneMat += LoadBoneTex(input.boneIndex.y, DrawFrame[input.instID].x,1) * input.weight.y;
	boneMat += LoadBoneTex(input.boneIndex.z, DrawFrame[input.instID].x,1) * input.weight.z;
	boneMat += LoadBoneTex(input.boneIndex.w, DrawFrame[input.instID].x,1) * input.weight.w;
...
PS_OUT PS(VS_OUT input)
{
	...
	//outColor.color = float4(Damy, 0,0,1);
	outColor.color.w = 1.0;
	float3 viewVec = ViewVec(input.worldPos);
	outColor.environment = environmentTex.Sample(smpWrap, float3(reflect(viewVec, normal)));
