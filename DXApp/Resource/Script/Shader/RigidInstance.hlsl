...
PS_OUT PS(VS_OUT input)
{
	...
	float3 H = normalize(normal + viewVec);
//	float3 ref = reflect(input.worldPos - EyePos, normal);
	outColor.environment = environmentTex.Sample(smpWrap, float4(H, 0));
	//outColor.environment = environmentTex.Sample(smpWrap, float4(reflect(viewVec, normal), 0));
	//outColor.environment = environmentTex.Sample(smpWrap, float4(normal.xyz, 1));
	outColor.normal = normalize( normal);
